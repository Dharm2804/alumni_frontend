...
const Chat: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!socketRef.current) {
      const newSocket = io('https://alumni-backend-6xcz.onrender.com', {
    ...
    const fetchCurrentUser = async () => {
      try {
        const response = await axios.get('https://alumni-backend-6xcz.onrender.com/api/current-user', {
    ...
    const fetchUsers = async () => {
      try {
        const response = await axios.get('https://alumni-backend-6xcz.onrender.com/api/chat/users', {
          withCredentials: true,
        });
        ...
        await Promise.all(users.map(async (user) => {
          ...
          try {
            const response = await axios.get(
              `https://alumni-backend-6xcz.onrender.com/api/chat/history/${roomId}`,
              { withCredentials: true }
            );
  ...
  const deleteAllChats = async () => {
    try {
      ...
      const response = await axios.delete('https://alumni-backend-6xcz.onrender.com/api/chat/delete-all', {
        headers: { email: localStorage.getItem('email') },
        withCredentials: true
      });
