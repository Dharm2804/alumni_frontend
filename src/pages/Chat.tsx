...
const Chat: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchCurrentUser = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/current-user`, {
        ...
        await Promise.all(users.map(async (user) => {
          ...
          try {
            const response = await axios.get(
              `${import.meta.env.VITE_API_URL}/api/chat/history/${roomId}`,
              { withCredentials: true }
            );
  ...
  const deleteAllChats = async () => {
    try {
      ...
      const response = await axios.delete(`${import.meta.env.VITE_API_URL}/api/chat/delete-all`, {
        headers: { email: localStorage.getItem('email') },
        withCredentials: true
      });
