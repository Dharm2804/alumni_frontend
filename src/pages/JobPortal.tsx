...
export default function JobPortal() {
  ...
  const [loading, setLoading] = useState(false); // New state for data fetching
  ...
  const handleDownloadJobs = async () => {
    ...
    try {
      ...
      const worksheetData = jobsData.map((job: Job) => ({
        'Job Title': job.title,
        Company: job.company,
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex gap-4">
            ...
            {userRole === 'faculty' && isLoggedIn && (
              ...
              >
                ...
                {downloadLoading ? 'Downloading...' : 'Download Jobs'}
              </motion.button>
            )}
          </div>
        </div>
        ...
        {(showAddForm || editingJob) && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
          >
            <motion.div className="absolute inset-0 bg-black bg-opacity-50" onClick={() => resetForm()} />
            <motion.div
