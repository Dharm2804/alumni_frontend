...
const Invitations: React.FC = () => {
  ...
  const fetchUserData = async () => {
    try {
      ...
      // Fetch user role
      const roleResponse = await axios.get('https://alumni-backend-6xcz.onrender.com/api/current-user', {
      ...
      // Fetch invitations
      const invitationsResponse = await axios.get<Invitation[]>('https://alumni-backend-6xcz.onrender.com/api/invitations', {
      ...
      if (roleResponse.data.role === 'faculty') {
        const alumniResponse = await axios.get<Alumni[]>('https://alumni-backend-6xcz.onrender.com/api/get_alumni', {
  ...
  const handleSendInvitation = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.post(
        'https://alumni-backend-6xcz.onrender.com/api/invitations/send',
  ...
  const handleResponse = async (invitationId: string, status: 'accepted' | 'rejected') => {
    try {
      ...
      await axios.put(
        `https://alumni-backend-6xcz.onrender.com/api/invitations/${invitationId}/respond`,
        { status },
        { headers: { email: roleResponse.data.email } }
      );
      fetchUserData();
