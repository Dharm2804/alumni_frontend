...
export default function VerifyOtp() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axios.post<{ token: string }>('https://alumni-backend-6xcz.onrender.com/api/verify-otp', { email, otp });
      setSuccess(true);
      setError('');
  ...
  const handleResendOtp = async () => {
    setIsLoading(true); // Start loading
    try {
      await axios.post('https://alumni-backend-6xcz.onrender.com/api/resend-otp', { email });
      setError('');
      setSuccess(true);
      setTimeout(() => {
        setSuccess(false);
