...
export default function VerifyOtp() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axios.post<{ token: string }>(`${import.meta.env.VITE_API_URL}/api/verify-otp`, { email, otp });
      setSuccess(true);
      setError('');
  ...
  const handleResendOtp = async () => {
    setIsLoading(true); // Start loading
    try {
      await axios.post(`${import.meta.env.VITE_API_URL}/api/resend-otp`, { email });
      setError('');
      setSuccess(true);
      setTimeout(() => {
        setSuccess(false);
